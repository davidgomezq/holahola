...
switch(_mode) do {
	...
	case 3: {
		...
		switch(_side) do {
			...
			case east: {_query = format["UPDATE players SET copeast_gear='%1' WHERE playerid='%2'",_value,_uid];};
		};
	};
	...
	case 9: {
		switch(_side) do {
			...
			case independent: {_query = format["UPDATE players SET med_gear='""[]""' WHERE playerid='%1'",_uid];};
		};
	};
};

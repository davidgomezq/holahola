...
if(_time <= 0) then { _time = time + (15 * 60); hintC "Error contacta con un admin: JAIL_FALLBACK_15, Tiempo en cero!"; };
...
while {true} do
{
	if((round(_time - time)) > 0) then
		...
		hintSilent parseText format[(localize "STR_Jail_Time")+ "<br/> <t size='2'><t color='#FF0000'>%1</t></t><br/><br/>" +(localize "STR_Jail_Pay")+ " %3<br/>" +(localize "STR_Jail_Price")+ " $%2",_countDown,[life_bail_amount] call life_fnc_numberText,if(isNil "life_canpay_bail") then {"Si"} else {"No"}];
	};
